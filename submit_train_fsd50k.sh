...
#SBATCH --job-name=audiosep_onepeace_train_fsd50k                     # sets the job name
#SBATCH --output=audiosep_onepeace_train_fsd50k.%j                    # indicates a file to redirect STDOUT to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
#SBATCH --error=audiosep_onepeace_train_fsd50k.%j                     # indicates a file to redirect STDERR to; %j is the jobid. If set, must be set to a file instead of a directory or else submission will fail.
